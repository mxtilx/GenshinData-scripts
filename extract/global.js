...
global.getTextMap = function (langcode) {
	if (textMapCache[langcode]) return textMapCache[langcode];
	...
	// Keep EN permanently, only keep one other language at a time
	if (langcode !== 'EN') {
		for (const key of Object.keys(textMapCache)) {
			if (key !== 'EN') delete textMapCache[key];
		}
	}
	textMapCache[langcode] = result;
	return result;
}
...
function exportDataByLang(folder, collateFunc, lang, englishonly, skipwrite) {
...
function exportData(folder, collateFunc, englishonly, skipwrite) {
	...
	langcodes.forEach(lang => {
		exportDataByLang(folder, collateFunc, lang, englishonly, skipwrite);
	});
...
module.exports = {
	...
	exportDataByLang: exportDataByLang,
	langcodes: langcodes
}
